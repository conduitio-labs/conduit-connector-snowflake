...
func NewPosition(insertElement int, updateElement int, deleteElement int) *Position {
	return &Position{
...
func ParseSDKPosition(p sdk.Position) (Position, error) {
	...
	if parts[0] != position.TypeCDC {
		return Position{}, position.ErrInvalidType
	}

	insertElement, err := strconv.Atoi(parts[1])
	if err != nil {
		return Position{}, position.ErrFieldInvalidElement
	}

	updateElement, err := strconv.Atoi(parts[2])
	if err != nil {
		return Position{}, position.ErrFieldInvalidElement
	}

	deleteElement, err := strconv.Atoi(parts[3])
	if err != nil {
		return Position{}, position.ErrFieldInvalidElement
	}
...
func (p Position) FormatSDKPosition() sdk.Position {
	return sdk.Position(fmt.Sprintf("%s.%d.%d.%d", p.Type,
		p.InsertElement, p.UpdateElement, p.DeleteElement))
}
